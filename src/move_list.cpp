...
void MoveList::add_moves(Square from, u64 targets, PieceType piece, const Position& position, u8 castling_rights,
                         Square en_passant_square, u8 halfmove_clock)
{
    while (targets)
    {
        ...
        add_move(Move{from, to, piece, type, captured, new_castling, en_passant_square, new_halfmove});
        targets &= targets - 1;
    }
}
...
void MoveList::add(Square from, Square to, PieceType piece, MoveType type, PieceType captured, u8 castling_rights,
                   Square en_passant_square, u8 halfmove_clock)
{
    add_move(Move{from, to, piece, type, captured, castling_rights, en_passant_square, halfmove_clock});
}
